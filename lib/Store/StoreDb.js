...
const saveContact = async (jid, name) => {
 try {
  if (!jid || !name || jid === undefined || name === undefined) return;
...
const saveMessage = async (message, user) => {
 try {
  ...
  const msg = message;
  if (!id || !jid || !msg || id === undefined || jid === undefined || msg === undefined) return;
...
const saveChat = async (chat) => {
 if (!chat || chat.id === undefined || chat.conversationTimestamp === undefined) return;
