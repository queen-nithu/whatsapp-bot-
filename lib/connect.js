...
const connect = async () => {
 ...
 const handleMessages = async ({ messages }) => {
  ...
  const executeCommand = (Instance, command, args) => {
  ...
  const canExecuteCommand = (command) => {
   if (config.WORK_TYPE === 'public') {
    if (command.fromMe) return msg.fromMe || msg.sudo || msg.devs;
    return true;
   } else if (config.WORK_TYPE === 'private') {
    return msg.fromMe || msg.sudo || msg.devs;
   }
   return false;
  };
